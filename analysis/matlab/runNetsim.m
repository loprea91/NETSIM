...
str = sprintf('make COMPILE_TYPE=%s -C%s', options.compile_type, options.netsimdir);
...
system(sprintf('make clean -C%s', options.netsimdir));
system(str);
system( sprintf('%s/netsim -f %s -o %s -j %u', options.netsimdir, paramfile, outputdir, jobid) ) ;
